...
*/
	var StylishCustom = {
		...
		setBtn: function () {
			document.documentElement.getButton("accept").label = (this.locale ? "儲存並關閉" : "Save and Close");
			...
			*/
			...
			with(IxUrlMenu) {
				id = "IxUrlMenu";
				setAttribute("label", this.locale ? "插入連結" : "Insert URL")
			}
			...
			IpopU.id = "DomRule";
			IpopU.setAttribute("label", this.locale ? "文檔規則" : "Document Rule");
		...
		showDocumentList: function (event, isChrome) {
			...
			if(!isChrome && menu.id != "chromeMenu"){
				...
				chromeMenu.setAttribute("label", this.locale ? "Chrome 路徑":"Chrome Url");
