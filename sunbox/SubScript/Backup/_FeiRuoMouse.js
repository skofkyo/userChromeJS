...
// @Note           [141128]修正Bing双引号未转义导致无法翻译的问题
// @Note           [141130][141202]小错误。
// @Note           [141206]修正bing保存Appid失败、baidu译文为英文单词无空格分隔的问题。
// @Note           [141211]切换默认翻译服务为baidu; baidu翻译添加(现代)英汉(大)辞典、中中、英英析义；修复bing变更导致获取Appid失败；
// @Note           [141212][141213]baidu翻译一些微调和修正。
// @Note           [141219]修复baidu英文在某些低版本FF下、google中英文、某些启用CSP的页面不发音的问题。
// @Note           [141220]同步baidu翻译更新，支持更多语言互译。
// @Note           [141221]修正HTML拼接错误。
// @Note           [141226]一些调整和修正。
// @Note           [150210]排除一些不可见元素的文本。
// @Note           [150309]应baidu网页翻译API更新，替代APP的API就能获取中中、英英析义，减少了必要的请求数，加快速度。bing改为https方式。
// @Note           [150328]baidu翻译增加某些英语单词大写时所表的不同意思。
// @Note           [150411]更新google API，更改默认google域名地址为cn， 并延长超时时间。
// @Note           [150619]同步baidu翻译更新，支持更多语言互译。
// @Note           [150623]修复更新 nightly41.050622 后，语法标准变更带来的错误。(http://whereswalden.com/2015/06/20/)
// ==/FireGesturesScript==
			...
			setTranslateText: function(word){
				...
				if(this.service == 'google'){
					details.url = this.google + 'translate_a/single?client=t&hl=auto&dt=bd&dt=ex&dt=ld&dt=md&sl=' + this.from + '&tl=' + this.to
										+ '&dt=qca&dt=rw&dt=rm&dt=ss&dt=t&dt=at&ie=UTF-8&oe=UTF-8&source=btn&srcrom=1&ssel=0&tsel=0&tk=|&q='
	...
	label: "强制开启阅读模式",
	command: function(event) {
		document.getElementById('reader-mode-button').click()
	}
    },{
		label: "转到左边标签页",
		command: function(event) {
