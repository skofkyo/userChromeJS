...
// @reviewURL      http://bbs.kafan.cn/thread-1685334-1-1.html
// @note              功能来自原脚本，我做了样式优化 by defpt
// ==/UserScript==
...
*/
	...
	_revertOldFindbar: true, //还原旧查找栏(默认关闭，若开启请将 false , 改为 true);
		...
		setBtn: function () {
			document.documentElement.getButton("accept").label = (this.locale ? "\u4FDD\u5B58\u5E76\u5173\u95ED" : "Save and Close");
			...
			with(IxUrlMenu) {
				id = "IxUrlMenu";
				setAttribute("label", this.locale ? "\u63D2\u5165\u94FE\u63A5" : "Insert URL")
			}
			...
			with(Ibtn) {
				id = "IxTextMenu";
				setAttribute("label", this.locale ? "\u63D2\u5165\u6587\u672C" : "Insert Text");
			}
			...
			IpopU.id = "DomRule";
			IpopU.setAttribute("label", this.locale ? "\u6587\u6863\u89C4\u5219" : "Document Rule");
		...
		lineNumber: function(){
			var observer = {
				observe: function(){
					...
					c.value = (StylishCustom.locale ? ",\u5217" : ",Col")+": " + lines[lines.length-1].length;
				}
			};
