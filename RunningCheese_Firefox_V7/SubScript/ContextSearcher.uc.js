...
// @version 0.0.8
// @note 0.0.8 Firefox 19 で入力栏で使えなくなったのを修正
// @note 0.0.8 NEW_TAB の初期値を browser.search.openintab にした
// @note 0.0.7 リンク上でも単语を取得するように変更
// @note 0.0.7 単语の取得を効率化
// @note 0.0.6 カタカナの正规表现のミスを修正
// @note 0.0.6 splitmenu をやめた（menu 部分をクリックして検索可能）
// @note 0.0.6 Mac でカーソル下の単语をうまく拾えてなかったらしいのを修正したかも
// @note 0.0.5 サブメニューを中クリックすると２回実行される问题を修正
// @note 0.0.5 メニューの検索エンジン名を非表示にした
// @note 0.0.5 カーソル下の単语の取得を调整
// @note 0.0.5 "?"をカタカナとして処理していたのを修正
// @note 0.0.4 アイコンの无い検索エンジンがあるとエラーになるのを修正
// ==/UserScript==
// http://f.hatena.ne.jp/Griever/20100918161044
// ホイールで既定のエンジン変更、サブメニューから他の検索エンジンの利用
// 右クリックの位置により选択文字列、カーソル下の単语を検索可能
...
// this.searchEngines[index-1] || this.searchEngines[this.searchEngines.length -1];
  ...
  setMenuitem: function() {
    ...
    this.menu.setAttribute('label', '"' + l + '" \u641c\u7d22');
