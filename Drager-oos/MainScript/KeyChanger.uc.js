...
// @note           0.0.2 Meta キーを裝飾キーに使えるようになったかもしれない（未テスト）
// @note           0.0.2 Windows キーを裝飾キーに使えるようになったかもしれない（未テスト Firefox 17 以降）
// ==/UserScript==
...
***** 書式について *****
keys["Ctrl+Shift+A"] = "alert(this)"; のように + で區切って記述。
不要なスペースは入れないでください。
Ctrl 等を使用しない場合は keys.VK_F1 でも可。

キーは大文字小文字區別せず、多少柔軟性を持たせてあります。
例：
	f1 → VK_F1
	Esc → VK_ESCAPE
...
***** 代入できるもの *****
string はそのまま oncommand 屬性に。

object は for in でそのまま屬性に。
	例：keys.f1 = { id : 'test', oncommand : 'alert(this.id)' };
	
function は "(" + func.toSource() + ").call(this, event);" で oncommand 屬性に。


スクリプトはこの辺を參考に調達してください。
