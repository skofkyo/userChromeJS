...
// @description  鼠標懸停打開鏈接（站外鏈接除外）,工具菜單開啟或鼠標手勢調用
// @author       ywzhaiqi
// @include      main
// @charset      utf-8
// @version      1.2
...
// @note         第一次需要從定製面板中拖出
// @note         當按著 Ctrl 鍵時，後台打開
//==/UserScript==

(function(){

    // 請不要在這裡更改，更改在圖標右鍵打開的設置裡。
    var
        BUTTON_TYPE = 2,  // 0：地址欄按鈕，1：可移動按鈕，2：菜單
        ENABLE = false,
        hovering_time = 500,  // 懸停的時間（毫秒）
    ...
    window.AutoClick = {
        ...
        loadSetting: function(type) {  // 載入設置並執行相應的任務
        ...
        updateState: function(enable, isTmpDisable) {
            let preLabel = isTmpDisable ? '臨時' : '',
                label = 'AutoClick 已' + preLabel + (enable ? '啟用' : '停用');
                ...
                \
                    ...
                    <hbox>\
                        ...
                        <menulist preference="BUTTON_TYPE">\
                          <menupopup>\
                            <menuitem label="地址欄按鈕" value="0"/>\
                            <menuitem label="可移動按鈕" value="1"/>\
                            <menuitem label="菜單" value="2"/>\
                          </menupopup>\
                        </menulist>\
                    </hbox>\
                    <label value="點擊的節點名：" />\
                    <textbox id="clicked_elem" flex="1" preference="clicked_elem" multiline="true" rows="3" />\
                    <label value="需要查找父元素的節點名：" />\
                    <textbox flex="1" id="clicked_parent_elem" preference="clicked_parent_elem" \
                        multiline="true" rows="3" />\
                    <button label="重置" onclick="reset();" tooltiptext="重置上面2個設置為默認值" />\
                \
                </prefpane>\
                <script>\
                    function reset() {\
