...
// Date 2009/08/06 00:00 tree_style_tab-0.8.2009073102があるとxulのdocument.overlayが出來なくなる件に対応
// Date 2009/05/23 00:00 userChrome.js0.8.1実験中 v3.0.25mod
// Date 2009/04/13 00:00 overlayのobserveの處理変更 v3.0.24mod
...
(function(){
  ...
  const AUTOREMOVEBOM   = false;  //BOMを自動的に, 取り除く:true, 取り除かない[false](元ファイルは.BOMとして殘る)
  ...
  var arrSubdir = new Array("", "MainScript", "SubScript", "BtnPlus", "userMenu", "xul");    //スクリプトはこの順番で実行される
  //===================================================================
 ...
 * chrome直下およびchrome/xxx.uc 內の *.uc.js および *.uc.xul
 * chrome/xxx.xul 內の  *.uc.js , *.uc.xul および *.xul
 * chrome/xxx.ucjs 內の *.uc.js は 特別に UCJS Loader 仕様を適用(NoScriptでfile:///を許可しておく)
 */

/* USE_0.63_FOLDER false の時
 *[ フォルダは便宜上複數のフォルダに分けているだけで任意。 下のarrSubdirで指定する ]
...
this.debug('Parsing getScripts: '+((new Date()).getTime()-Start) +'msec');
    ...
    */
    ...
    //Fxのバージョンを 數値で返す 3.0 3.5 とか 3.6とか
    getVer: function(){
