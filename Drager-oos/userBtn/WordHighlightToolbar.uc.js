...
// @include        main
// @version        0.0.8
// @downloadURL    https://github.com/ywzhaiqi/userChromeJS/tree/master/WordHighlightToolbarFix
// @reviewURL      https://github.com/Griever/userChromeJS/tree/master/WordHighlightToolbar
// @note           增加延遲及 super_preloader 加載下一頁高亮的支持 By ywzhaiqi
// @note           0.0.8 Firefox 25 でエラーが出ていたのを修正
// @note           0.0.7 ツールバーが自動で消えないことがあったのを修正
// @note           0.0.6 アイコンを作って検索時の強調を ON/OFF できるようにした
// @note           0.0.6 背面のタブを複數開いた際の引き継ぎを修正
...
var USE_FIX_AUTOPAGER = true;  // 增加通用檢測下一頁後重新高亮，用 uAutoPagerize 不需要
var disableBooklink = true;  // 來自 booklink.me 的百度搜索不要高亮。
...
window.gWHT = {
	...
	set GET_KEYWORD(bool) {
		...
		if(menuitem){
			...
			menuitem.setAttribute("label", "搜索高亮工具例已" + (bool ? "啟用" :"禁用"));
		}
  ...
  //       icon.setAttribute("onclick", "gWHT.iconClick(event);");
		...
		//menuitem.setAttribute("label", "高亮工具例開 / 關");
		menuitem.setAttribute("tooltiptext", "高亮工具例功能開 / 關");
		...
		menu.setAttribute("label", "高亮關鍵詞");//ハイライト
		...
		var menuitem = menupopup.appendChild(document.createElement("menuitem"));
		menuitem.setAttribute("class", CLASS_ICON + " menuitem-iconic");
		menuitem.setAttribute("label", "分色高亮不同關鍵詞");//単語を探してハイライト
	...
	addToolbar: function() {
		...
		closebutton.setAttribute("tooltiptext", "關閉高亮工具例");
		...
		addbutton.setAttribute("tooltiptext", "添加關鍵詞");//ワードを追加
	...
	addButton: function(toolbar, aItem, aButton) {
		...
		if (!button) {
			...
			button.setAttribute('tooltiptext', [
				'單擊/滾輪向下 - 轉到下一個高亮處',//クリック or ホイールダウンで次を検索
				'Shift+單擊/滾輪向上 - 轉到上一個',//Shift+クリック or ホイールアップで前を検索
				'中鍵單擊 - 取消高亮效果'].join('\n'));//ホイールクリックで削除
	...
	addWord: function(aWord, aBold, aRange) {
		if (!aWord) {
			aWord = prompt('^o^ 請輸入關鍵詞：', getBrowserSelection());
			aBold = true;
		}
		if (!aWord) return;
	...
	get tangoReg() {
		...
		var arr = [
			...
			,"[\\u4E00-\\u9FA0][\\u3040-\\u309F]+" // 漢字１文字＋ひらがな
			,"[\\u30A0-\\u30FA\\u30FC]{2,}" // カタカナ
			,"[\\uFF41-\\uFF5A\\uFF21-\\uFF3A\\uFF10-\\uFF19]{2,}" // 全角英數數字（小文字、大文字、數字）
			,"[\\w%$\\@#+]{5,}"
			,"\\d[\\d.,]+"
			,"\\w[\\w.]+"
		];
...
window.gWHT.ContentClass.prototype = {
	...
	init: function(doc, keywords) {
		...
		this.finder.caseSensitive = false; /* 大文字小文字を區別するか */
	...
	handleEvent: function(event) {
		switch (event.type) {
			...
			case "GM_AutoPagerizeNextPageLoaded":
				// AutoPagerizeの最後の區切り以降のRangeを取得
...
list-style-image: url("data:image/png;base64,");\
}\
.wordhighlight-toolbar-addbutton {\
  list-style-image: url("data:image/png;base64,");\
}\
