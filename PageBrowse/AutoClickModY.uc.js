...
// @note         第一次需要从定制面板中拖出
// @note         鏈接改為使用中鍵點擊 其他的依然是左鍵點擊 by skofkyo
...
(function(){
    ...
    window.AutoClick = {
        ...
        mouseover: function(event) {
            var tar = event.target.localName == 'a';
            var elem = this.findLink(event.target);
            if (elem && tar) {
                this.timeoutID = setTimeout(function() {
                    AutoClick.Mclick(elem, event.view);
                }, this.hovering_time);
            }
            if (elem && !tar) {
                this.timeoutID = setTimeout(function() {
                    AutoClick.Lclick(elem, event.view);
                }, this.hovering_time);
            }
        },
        ...
        Mclick: function(element, win) {
            let doc = win.document,
                e = doc.createEvent('MouseEvents');
            e.initMouseEvent('click', true, true, win, 0,
                0, 0, 0, 0, this.ctrlKey, false, false, false, 1, element);
            return !element.dispatchEvent(e);
        },
        Lclick: function(element, win) {
            let doc = win.document,
                e = doc.createEvent('MouseEvents');
            e.initMouseEvent('click', true, true, win, 0,
