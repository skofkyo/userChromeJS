...
window.siteinfo_writer = {
	        ...
	        <window id="main-window">\
				<vbox id="sw-container" class="sw-add-element" hidden="true" collapsed="false">\
				    <hbox id="sw-hbox" collapsed="false">\
				      <toolbarbutton label="查看規則" oncommand="siteinfo_writer.toJSON();"/>\
				      <toolbarbutton label="查看規則(SP)" oncommand="siteinfo_writer.toSuperPreLoaderFormat();"/>\
				      <toolbarbutton id="sw-curpage-info" label="讀取當前頁面規則" oncommand="siteinfo_writer.getCurPageInfo();"/>\
				      <toolbarbutton label="從剪貼板讀取規則" oncommand="siteinfo_writer.readFromClipboard();"/>\
				      <toolbarbutton id="sw-launch" label="啟動規則" tooltiptext="啟動uAutoPagerize" oncommand="siteinfo_writer.launch();"/>\
	...
	toJSON: function() {
		...
		json += "\t},";
		var r=confirm("翻頁規則（按確定鍵將其複製到剪貼板）："+'\n\n' + json);
		if(r){
			try{
	...
	toSuperPreLoaderFormat: function() {
        ...
        spdb += "\t\t}\n";
        spdb += "\t},";
        var r = confirm("翻頁規則(SuperPreLoader格式)（按OK鍵將其複製到剪貼板）：" + '\n\n' + spdb);
        if (r) {
            try {
	...
	getCurPageInfo: function(){
		...
		if(list.length == 1){
			...
			for (let [i, info] in Iterator(list)) {
                ...
                menuitem.setAttribute("tooltiptext", "右鍵點擊複製");
